...
const useRecuperarPizzasComPaginacao = (queryString: QueryString) => {
  
  const recuperarPizzasComPaginacao = async (queryString: QueryString) => {
    ...
    const response = await fetch("http://localhost:8080/pizzas/paginacao?" + 
      new URLSearchParams({
    ...
    return (await response.json()) as ResultadoPaginado<Pizza>; // asserção de tipo
  };

  return useQuery({
    // É preciso acrescentar o return antes de useQuery.
    // Veja que na linha 5 o return está implícito.
    queryKey: ["pizzas", "paginacao", queryString],
    queryFn: async () => recuperarPizzasComPaginacao(queryString),
    staleTime: 0,
    placeholderData: keepPreviousData, // mostrar com e sem isso. Importante manter o setTimeout.
  });
};
export default useRecuperarPizzasComPaginacao;
